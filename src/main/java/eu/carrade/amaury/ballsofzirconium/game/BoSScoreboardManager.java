...
public class BoSScoreboardManager extends QuartzComponent implements Listener
{
    ...
    public void updateTimer()
    {
        ...
        if (!BallsOfZirconium.get().getBarManager().isEnabled() && BallsOfZirconium.get().getGameManager().isGameRunning())
        {
            timerText = I.t("{0}{gray}│ {gold}{1}", sidebarTitle, getTimerText(BallsOfZirconium.get().getGameManager().getTimer()));
        }
        else
        {
