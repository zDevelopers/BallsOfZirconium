...
import eu.carrade.amaury.BallsOfSteel.generation.generators.helpers.WithPatternGenerator;
import eu.carrade.amaury.BallsOfSteel.generation.postProcessing.PostProcessor;
import eu.carrade.amaury.BallsOfSteel.generation.postProcessing.ReplacePostProcessor;
import eu.carrade.amaury.BallsOfSteel.generation.postProcessing.SetPostProcessor;
...
public class CheckGenerationSettings extends SpheresRelatedCommand
{
	...
	protected void run() throws CommandException
	{
		...
		for (final GeneratedSphere sphere : BallsOfSteel.get().getGenerationManager().getSpheres())
		{
			errorsCount += check(sphere.getName(), new HashSet<>(sphere.getGenerators()), new HashSet<>(sphere.getPostProcessors()));
		}

		for (final StaticBuilding building : BallsOfSteel.get().getGenerationManager().getBuildings())
		{
